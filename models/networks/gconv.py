...
class SetConv(torch.nn.Module):
    def __init__(self, nb_feat_in, nb_feat_out):
        """
        Module that performs PointNet++-like convolution on point sets.
        Adapted from https://github.com/valeoai/FLOT/blob/master/flot/models/gconv.py
    ...
    def forward(self, signal, graph):
        ...
        graph : flot.models.graph.Graph
            Graph build on the input point set on with the input features 
        ...
        signal = torch.cat((signal[graph.edges], graph.edge_feats), -1) #(bs*n*k, c+3)
