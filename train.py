...
for epoch in iter_counter.training_epochs(): 
    ...
    for i, data_i in enumerate(dataloader, start=iter_counter.epoch_iter):
        ...
        # get data
        identity_points, pose_points, id_face, pose_face = data_i

        # training
        trainer.train_model(identity_points, pose_points, id_face, arap_flag)
        ...
        if iter_counter.needs_displaying():  
            try:
                ...
                pymesh.save_mesh_raw(save_root + '/' + str(epoch) + '_' + str(iter_counter.total_steps_so_far) + '_fake_id.obj', 
                                                trainer.out['fake_id_points'][0,:,:].cpu().detach().numpy().transpose(1,0),id_face[0,:,:].cpu().numpy())
                pymesh.save_mesh_raw(save_root + '/' + str(epoch) + '_' + str(iter_counter.total_steps_so_far) + '_fake_pose.obj', 
                                                trainer.out['fake_pose_points'][0,:,:].cpu().detach().numpy().transpose(1,0),pose_face[0,:,:].cpu().numpy())  
